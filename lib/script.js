...
Vue.component('chordbook-app', {
  ...
  mounted() {
    ...
    this.$watch(
      () => this.song.title,
      (title) => (document.title = `${title} — Shhh! KANGHONG`),
      { immediate: true }
    )
    requestAnimationFrame(frame)
  },
  ...
  template: `<div>
    <nav class="navbar navbar-expand-lg navbar-light bg-light">
      <span class="navbar-brand">Shhh! KANGHONG <small class="text-muted" v-if="devMode"> Thanks repository from dtinth</small></span>
    </nav>
